...
const sfaceModelPath = "models/face_recognition_sface_2021dec.onnx"

func featureFor(t *testing.T, det *Detector, rec *Recognizer, imgPath string) []float32 {
	t.Helper()
...
// (same algorithm this package's AlignCrop/Feature/Match were ported from)
// on testdata/amy.jpg and testdata/bernadette.jpg, both detected on the
// same 640x640 letterboxed canvas this package's Detector uses: opencv-python
// reported cosine(amy,amy)=1.0, cosine(amy,bernadette)=0.1217,
...
func TestRecognizerMatchesPythonReference(t *testing.T) {
	...
	cosSame := Match(amyFeat, amyFeat, DistanceCosine)
	if math.Abs(cosSame-1.0) > 1e-4 {
		t.Errorf("cosine(amy,amy) = %v, want ~1.0", cosSame)
	}

	cosDiff := Match(amyFeat, bernFeat, DistanceCosine)
	...
	l2Same := Match(amyFeat, amyFeat, DistanceL2)
	if l2Same > 1e-4 {
		t.Errorf("l2(amy,amy) = %v, want ~0.0", l2Same)
	}

	l2Diff := Match(amyFeat, bernFeat, DistanceL2)

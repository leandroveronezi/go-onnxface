...
// sfaceTemplate is the canonical 112x112 landmark template
// (right eye, left eye, nose tip, right corner of mouth, left corner of
// mouth) that alignCrop warps detected landmarks onto. Taken verbatim from
// OpenCV's FaceRecognizerSFImpl::getSimilarityTransformMatrix.
var sfaceTemplate = [5][2]float64{
	{38.2946, 51.6963},
	{73.5318, 51.5014},
	{56.0252, 71.7366},
	{41.5493, 92.3655},
...
// used by the rank check in getSimilarityTransformMatrix.
...
landmarks (as returned by Detector), matching
cv::FaceRecognizerSF::alignCrop: a similarity transform (rotation, scale,
translation -- no perspective) is estimated from the landmarks onto a fixed
template, then applied with bilinear interpolation.
*/
...
/*
similarityTransform estimates the 2x3 affine matrix mapping src (5 points)
onto sfaceTemplate, via Umeyama's method. Ported line-by-line from
cv::FaceRecognizerSFImpl::getSimilarityTransformMatrix so it produces the
same matrix (rotation + uniform scale + translation, with a reflection
correction when needed) as OpenCV does.
*/
